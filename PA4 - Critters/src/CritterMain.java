  ...
  */
    ...
    public synchronized void update() {
        ...
        for (int i = 0; i < critterList.size(); i++) {
            ...
            if (debug)
                System.out.println("  processing critter: " + Util.toString(critter1) + " at " + Util.toString(location));
            ...
            if (critter2 == null) {
                ...
                if (critter1.getClass() == critter2.getClass()) {
                    ...
                    if (!critterState1.hasMate() && !critterState2.hasMate() &&
                            !critterState1.isBaby() && !critterState2.isBaby()) {
                        // they fall in love!
                        if (debug)
                            System.out.println("    mating begins at " + Util.toString(location) + " between " + Util.toString(critter1) + " and " + Util.toString(critter2));
                    ...
                    if (debug)
                        System.out.println("      winner=" + Util.toString(winner) + ", loser=" + Util.toString(loser));
            ...
            if (winner == critter1) {
                ...
                if (CRITTER_MOVE_FATIGUE_COUNT > 0 && critterState1.moves % CRITTER_MOVE_FATIGUE_COUNT == 0) {
                    ...
                    if (debug)
                        System.out.println("    moved too much; falling asleep for " + GLUTTON_PENALTY + " moves");
                    ...
                    if (critterEat) {
                        ...
                        if (CRITTER_GLUTTON_COUNT > 0 && critterState1.foodEaten % CRITTER_GLUTTON_COUNT == 0) {
                            critterState1.foodPenalty = GLUTTON_PENALTY;
                            if (debug)
                                System.out.println("    ate too much; falling asleep for " + GLUTTON_PENALTY + " moves");
                            critter1.setAwake(false);
                            critter1.sleep();
                        }
                        ...
                        if (CRITTER_CLASS_GLUTTON_COUNT > 0 && classState1.foodEaten % CRITTER_CLASS_GLUTTON_COUNT == 0) {
                            ...
                            if (debug)
                                System.out.println("    class ate too much; falling asleep for " + GLUTTON_PENALTY + " moves");
                            critter1.setAwake(false);
                            critter1.sleep();
                        }
                    }
        ...
        for (CritterState state : critterStates) {
            if (state.isAsleep()) {
                ...
                if (state.isAsleep()) {
                    if (debug)
                        System.out.println("  " + state.foodPenalty + " moves until wakeup for " + Util.toString(state.critter));
            ...
            if (state.isMating()) {
                ...
                if (state.isMating()) {
                    if (debug)
                        System.out.println("  " + state.matePenalty + " moves until done mating for " + Util.toString(state.critter) + " and " + Util.toString(state.mate));
                } else {
                    // new baby born!
                    ...
                    if (debug)
                        System.out.println("  done mating for " + Util.toString(state.critter) + " and " + Util.toString(state.mate));
    ...
    private Critter add(Class<? extends Critter> critterClass, Point location)
        ...
        if (debug)
            System.out.println("Constructed new " + critterClass.getName() + " (id " + obj.hashCode() + ") at " + Util.toString(location) + " with parameter(s): " + Arrays.toString(params));
